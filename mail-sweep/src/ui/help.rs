...
fn help_lines() -> Vec<Line<'static>> {
    vec![
        ...
        blank(),
        heading("Workflow"),
        plain("  1. s — sync new/unread mail from Gmail (UI stays interactive)"),
        ...
        plain("     Clearing (archive/move/delete) also marks mail read so category"),
        plain("     tabs (Promotions, Social, Updates, Purchases) drop unread badges."),
        plain("  5. A — AUTO: repeat sync → AI → safe apply on interval"),
        plain("     Sync / classify / apply / AUTO run in the background — navigate freely."),
        blank(),
        heading("Triage tab"),
        ...
        plain("If both lists exist, u switches between pending senders and unread leftovers."),
        key("j/k", "Select sender group or leftover message"),
        key("u", "Toggle pending senders ↔ unread leftovers (when both exist)"),
        key("Enter", "Read selected mail (leftover or sender sample, cached body)"),
        ...
        blank(),
        heading("Reading mail"),
        plain("Enter opens a scrollable reader for the selected leftover or the selected"),
        plain("sender's sample message. Esc closes. m marks \\Seen on the server"),
        plain("and removes it from the Unread list. M marks all leftovers read at once"),
        plain("(works even while the sender list is showing). Teach keys work in the reader too."),
        blank(),
        heading("Review tab"),
        ...
        key("Enter", "Read cached body — check whether the planned action/rule fits"),
        ...
        key("s", "Sync mail now"),
        plain("Teach keys also work inside the reader (Esc closes)."),
        blank(),
...
pub fn tab_hint(tab: Tab) -> Line<'static> {
    let text = match tab {
        Tab::Triage => "Triage — Enter read · teach senders · u unread leftovers · m / M mark read",
        Tab::Review => "Review — Enter read · z/g/i/o correct · r reject · a apply plan",
